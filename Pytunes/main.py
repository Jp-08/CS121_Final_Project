...
def add_track():
    ...
    def submit_track():
        ...
        try:
            ...
            cursor = conn.cursor()
            cursor.execute("""
                INSERT INTO tracks (title, artist, album, genre, release_year)
                VALUES (?, ?, ?, ?, ?)
...
def update_track():
    ...
    def submit_update():
        ...
        cursor.execute("""
            ...
            WHERE id = ?
        """, (title, artist, album, genre, release_year, track_id))
        conn.commit()
        conn.close()
...
# Function to refresh the track library
def refresh_library():
    refresh_tracks()  # Call the function that updates the track list
    messagebox.showinfo("Library Refreshed", "Track library has been refreshed successfully!")
